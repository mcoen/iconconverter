...
def run(arguments):
    ...
    icon_font = IconFont(cssFile=args.css.name,
                         ttfFile=args.ttf.name,
                         keepPrefix=args.keepPrefix)
    args.css.close()
    args.ttf.close()

    # Then '--list'
    if args.list:
        for icon in icon_font.cssIcons.keys():
            print(icon)
        parser.exit()
    ...
    if not args.icons:
        ...
        selected_icons = icon_font.cssIcons.keys()
    else:
        for icon in args.icons:
            if (args.keepPrefix and
                    not icon.startswith(icon_font.commonPrefix)):
                # Prepend icon name with prefix
                icon = icon_font.commonPrefix + icon
            elif (not args.keepPrefix and
                  icon.startswith(icon_font.commonPrefix)):
                # Remove prefix from icon name
                icon = icon[len(icon_font.commonPrefix):]

            # Check if given icon names exist
            if icon in icon_font.cssIcons:
